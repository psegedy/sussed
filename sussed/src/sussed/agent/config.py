"""
Search configuration for autonomous agent mode 🎯

Define what you're looking for and let the agent hunt for it.
"""
...
class SearchCriteria(BaseModel):
    ...
    max_floor: int | None = Field(default=None, description="Max floor (avoid top floor = leaky roof)")
    ...
    reject_ground_floor: bool = Field(default=False, description="No ground floor (louder, less safe)")
    ...
    def get_exclude_keywords(self) -> list[str]:
        ...
        if not self.exclude_description_keywords:
            return []
        return [k.strip().lower() for k in self.exclude_description_keywords.split(",") if k.strip()]
