...
def valid_review_payload(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "score_reason": "A solid listing with a reasonable price for the district. [https://www.sreality.cz/detail/prodej/byt/2-kk/brno/000000]",
...
def test_review_result_accepts_full_structured_payload() -> None:
    payload = ReviewResultInput(
        ...
        confidence=0.88,
        recommendation="CONSIDER",
        score_reason="Below-market 2+kk with good light, but parking costs extra. [https://www.sreality.cz/detail/prodej/byt/2-kk/brno/123]",
...
def test_review_result_rejects_invalid_scores(score: int) -> None:
    with pytest.raises(ValidationError):
        ReviewResultInput(
            ...
            confidence=0.5,
            recommendation="SKIP",
            score_reason="Invalid score should fail. [https://www.sreality.cz/detail/x]",
            summary="Invalid.",
            reviewer_name="sussed-ai-review",
            input_hash="abc123",
        )
...
def test_review_result_accepts_special_scores() -> None:
    sus = ReviewResultInput(
        ...
        score_reason="Likely scam. [https://www.sreality.cz/detail/prodej/byt/2-kk/brno/000]",
        summary="Avoid.",
        reviewer_name="sussed-ai-review",
        input_hash="abc123",
    )
    gem = ReviewResultInput(
        ...
        confidence=0.95,
        recommendation="BUY",
        score_reason="Exceptional deal. [https://www.sreality.cz/detail/prodej/byt/2-kk/brno/001]",
        summary="Unicorn.",
        reviewer_name="sussed-ai-review",
        input_hash="def456",
    )
